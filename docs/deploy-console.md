...
1536 и нормализира наново, защото това е таванът на Vectorize.

Числото стои на едно място — `EMBED_DIMENSIONS` в `src/lib/constants.ts` — и
трябва да съвпада с индекса. Ако някога го смениш, индексът трябва да се
пресъздаде: ширината на съществуващ не се мени.
