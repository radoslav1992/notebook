...
| 9 | **`zapiski.bg` в Cloudflare Email Service** | потвърждаване на имейл, нова парола, форма за контакт | Не — но без него писмата не излизат; виж [email.md](email.md) |
...
по потребление (центове при десетки въпроси) + писмата през Cloudflare Email
Service + Stripe само процент от оборота.

---

## 2. Локална работа за 5 минути

Дотук стига без Google, без доставчик на имейли и без Stripe:
...
се показват в интерфейса, докато няма настроен доставчик. Това е нарочно, за да е
ползваемо локално.
...
`0004` слага външни ключове на таблиците от `0002`, които бяха без такива, и по
пътя изхвърля вече осиротелите редове. Пресъздава четири таблици (SQLite не може
да добави ограничение към съществуваща), затова я пусни, когато никой не работи.

Миграциите се пускат наново след всяко обновяване, което носи нова. `0003`
добавя индекса за търсене по думи и **сам вписва вече качените пасажи** — не е
нужно нищо да се преиндексира. Докато не е пусната, отговорите излизат само с
векторното търсене (в лога се вижда `[zapiski:fts]`), тоест нищо не спира, но
буквалните съвпадения — „чл. 21“ — стават по-слаби.

> Едно следствие: `wrangler d1 export` не поддържа виртуални таблици. Ако някога
> правиш резервно копие с него, изтрий `chunks_fts` преди това и пусни `0003`
> наново след това — тя се възстановява от `chunks` сама.
...
Нужни са за три неща: потвърждаване на имейл, нова парола и формата за контакт.

**Доставчикът по подразбиране е Cloudflare Email Service** — binding-ът
`send_email` стои в `wrangler.jsonc`, тоест няма ключ за слагане. Остава само
домейнът:

1. `zapiski.bg` да е зона в същия Cloudflare акаунт.
2. **Email → Email Service → Domains** → добави `zapiski.bg` и сложи показаните
   DNS записи (DKIM, SPF, обикновено и DMARC).
3. **Email Routing → Destination addresses** → потвърди адреса, на който да идва
   формата за контакт.

> Стъпка 2 е тази, която отваря **произволни** получатели. Преди нея binding-ът
> праща само до потвърдени адреси — стига за формата за контакт, но не и за
> потвърждаване на имейл на истински потребител.

Пълните подробности, включително двата различни адреса за контакт и как е
защитена формата, са в [`email.md`](email.md).

**Resend остава като алтернатива**, ако предпочетеш него: сложи
`RESEND_API_KEY` и приложението го ползва само когато binding-ът липсва.

```bash
npx wrangler secret put RESEND_API_KEY   # само ако не ползваш Cloudflare
```

Без доставчик профилите се създават, но остават непотвърдени, а „забравена
...
| `EMAIL_FROM` | `Записки <noreply@zapiski.bg>` | Подател на писмата; адресът трябва да е на onboard-нат домейн. |
| `CONTACT_EMAIL` | `info@zapiski.bg` | Публичният адрес, който се показва на `/contact`. |
| `CONTACT_TO` | `dev.radoslav.dodnikov@gmail.com` | Където отиват съобщенията от формата. |
...
| 3 | Направи профил | Влиза направо в `/app`; при настроен доставчик идва писмо |
...
| 13 | `/contact` → изпрати съобщение | Пристига на `CONTACT_TO`, „Отговори“ отива при пишещия |
| 14 | Настройки → „Изтегли данните си“ | Сваля `zapiski-ГГГГ-ММ-ДД.json` с тетрадките и разговорите |
| 15 | Настройки → „Изтрий профила“ | Иска паролата, после изхвърля на `/`; влизане със същия имейл вече не намира профил |

Ако нещо от 1–6 не мине, проблемът е в Cloudflare или Gemini. 7–8 е Google или
имейлите ([email.md](email.md)). 10–12 е Stripe.

Стъпки 3 и 4 вървят заедно при настроен доставчик на писма: докато имейлът не е
потвърден, новата тетрадка се отказва с 403 и текст, който сочи към Настройки.
Потвърди от писмото (или от „Изпрати наново“) и опитай пак.

Стъпка 15 е необратима — направи я с профил за еднократна употреба.
